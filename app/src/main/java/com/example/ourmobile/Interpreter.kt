...
class Expression {
    public fun toReversePolishNotation(
        expressionString: String,
        variables: Map<String, Any>,
        program: CelestialElysiaInterpreter
    ): String {
        ...
        val tokenRegex =
            Regex("(((?<=[+\\-\\/*]))-\\d+(\\.\\d+)?|[A-Za-z]+\\w*(\\[.+\\])|[+\\-\\/*)(]|\\w+<[\\w:+\\-\\/*\\|\\.]+>|[a-zA-Z]\\w*\\.[a-zA-Z]\\w*\\[.+\\]|[a-zA-Z]\\w*\\.[a-zA-Z]\\w*|\\w+(\\.\\d+)?)")
        ...
        for (match in matches) {
            ...
            if (token in operators) {
                while (stack.isNotEmpty() && stack.last() in operators && precedence(stack.last()) >= precedence(
                        token
                    )
                ) {
                ...
                if (token.toDoubleOrNull() != null) {
                    ...
                    for (n in functionArguments) {
                        ...
                        if (functionProgram!!.varHashMap[name]!!::class.java.simpleName == "Integer") {
                            ...
                            functionProgram!!.varHashMap.put(
                                name,
                                program.stack.removeFirst().toInt()
                            )
...
fun boolExpression(
    expression1: String, expression2: String, operand: String,
    program: CelestialElysiaInterpreter
): Boolean {
    var tokenRegex = Regex("<\\w+")
    var token1Name = tokenRegex.find(expression1)!!.value
    var token1Type = program.tokenHashMap.get(token1Name)
    var token1Object = token1Type?.java?.newInstance() as? IToken ?: throw IllegalArgumentException(
        "Invalid token type"
    )
    ...
    var token2Object = token2Type?.java?.newInstance() as? IToken ?: throw IllegalArgumentException(
        "Invalid token type"
    )
